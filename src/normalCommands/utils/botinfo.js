...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const embed = new EmbedBuilder()
      .setTitle(`Olá ${message.author.tag}, minhas informações estão abaixo!`)
      .setColor('A020F0')
    ...
    message.channel.send({ embeds: [embed], components: [buttons, linksuporte] }).then(msg => {
      ...
      collector.on("collect", async (interaction) => {
        ...
        switch (interaction.customId) {
          case '1': {
            embed.setTitle(` Olá ${message.author.tag}, minhas informações estão abaixo! `)
            embed.setColor('A020F0')
            embed.setDescription(`\`Informações do BOT\`\n\nMeu prefixo é \`${prefixo}\`\nPing do WebSocket: \`${ping}\`ms\nEstou em \`${majusv}\` servidores\nAcessando \`${channels}\` canais\nModerando \`${users}\` usuários!\nEstou online há: \`${dias}\` dias\nEstou online há: \`${horas}\` horas\nEstou online há: \`${minutos}\` minutos\nEstou online há: \`${segundos}\` segundos.\n\nInformações sobre a máquina:\n`)
            embed.setFooter({
              text: `Comando requisitado por: ${message.author.tag}`,
              iconURL: message.author.displayAvatarURL({ format: "png" })
            }
            );
            msg.edit({ embeds: [embed], components: [buttons], ephemeral: true });
            await interaction.deferUpdate();
            break;
          }
          case '2': {
            const twoembed = new EmbedBuilder()
            twoembed.setTitle(` Olá ${message.author.tag}, informações do desenvolvedor está abaixo! `)
            ...
            msg.edit({ embeds: [twoembed], components: [buttons] })
            await interaction.deferUpdate();
            break;
          }
